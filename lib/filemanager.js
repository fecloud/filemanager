 ...
 * 返回当前目录的子目录以及文件
 */
function list_dir(req, res, params) {
    ...
    if (params.value) {
        ...
        fs.readdir(dir, function (err, files) {
            if (err) {
                ...
                //有按页加载
                if (params.skip) {
                    if (files.length > params.skip) {
                        var num = params.num;
                        var dir_files = get_files_info(dir, params.value, files);
                        if (num && num > 0) {
                            for (var i = params.skip; i < files.length; i++) {
                                num--;
                                array_files.push(dir_files[i]);
                                if (num == 0) {
                                    more = (i + 1) < files.length -1
                                    break;
                                }
                            }
                    ...
                    //返回所有
                    array_files = get_files_info(dir, params.value, files);
                }
